...
public class GenerateQueries {
    ...
    String url = "http://10.204.21.38:8080/CloudWebService/rest/result?query=";
    ...
    public ArrayList<ArrayList<String>> getPossibilities() {
        ...
        try {
            attributes = process("show columns in five");
        ...
        {
            try {
                attributePossibilities = process("SELECT " + possibilities.get(0).get(i-1) + " FROM five"); //Gets every entry for an attribute in the table
            } catch(JSONParserException | DownloadDataException e){}
    ...
    public String generateNum(String attribute, ArrayList<String> possibilities)
    {
        ...
        return "SELECT * FROM five WHERE " + attribute + " " + operators.get((int)(Math.random() * operators.size())) + " " + possibilities.get((int)(Math.random() * (possibilities.size()))) + ";";
    }

    public String generateString(String attribute, ArrayList<String> possibilities)
    {
        return "SELECT * FROM five WHERE " + attribute + " = '" + possibilities.get((int)(Math.random() * (possibilities.size()))) + "';";
    }
    ...
    public String generateDate(String attribute, ArrayList<String> possibilities)
    {
        ...
        if(date_or_time == 0)
        {
            return "SELECT * FROM five WHERE substr(" + attribute +  ",0,10) " + operators.get((int)(Math.random() * operators.size())) + " %27" + possibilities.get((int)(Math.random() * (possibilities.size()))).substring(0,10)+"%27;";
        }
        else
        {
            return "SELECT * FROM five WHERE substr(" + attribute + ",12) " + operators.get((int)(Math.random() * operators.size())) + " %27" + possibilities.get((int)(Math.random() * (possibilities.size()))).substring(11) + "%27;";
        }
    }
